...
status: vollständig
fortschritt: 100
zusammenfassung: Pfad- und Namensfallen, fünfzehn Widersprüche zwischen Doku, Code und Messung, Verweise. Nr. 5 ist am 12.09.2026 aufgelöst: Die Eigenmessung vom 27.08. rechnete Token gegen Grund, Lighthouse sah die gerenderte Seite -- beide Male lag der Fehler dazwischen, zuerst in der hellen Fassung von --ink-dim, dann in einem opacity:.5 über der richtigen Farbe. Eine Eigenmessung an Tokens beweist nichts über das, was auf dem Bildschirm ankommt.
offen: 0
...
- **Diese Seite ist die grösste im Bestand und liegt am ungewöhnlichsten.** 165 URLs — mehr als Rümpelwerk (85) und fast zehnmal so viele wie RTC-Service (17) —, und als einzige der sechs betreuten Seiten liegt ihr Projektordner nicht unter `Desktop\webseiten buisnes\`, sondern im JARVIS-Baum.
- **Dreisprachig ohne gettext.** DE/EN/RO über eigene `PACK`-Dicts in `landing/i18n/`, kein `compilemessages`, kein `.po`/`.mo`. `de.py` ist Master; EN und RO erben per Deep-Merge, **aktuell erbt kein einziger Schlüssel** (rund 800 Schlüssel je Sprache, geprüft von `pruefe_seite`). Weil die Pakete vertrauenswürdige HTML-Entities enthalten, werden `{{ t.* }}`-Ausgaben mit `|safe` gerendert — daher die 326 Befunde „Ausgabe ohne Maskierung" im Code-Audit; das ist Absicht, keine Lücke.
...
- **Die Sitemap kann recht haben und der Seitenkopf trotzdem lügen.** Die Einsprachigkeit der drei Silos war im vierten Feld von `_seiten_pfade()` sauber modelliert — die Sitemap meldete keine `/en/wissen/…`-Adresse. Der **Seitenkopf** derselben Seiten gab bis zum 05.09.2026 trotzdem `hreflang="en"` auf genau diese Adressen aus, und die antworten mit 404. Google verwirft eine hreflang-Gruppe, in der ein Glied nicht auflöst, **vollständig**. Behoben über `i18n.hat_sprachfassung()`, das den URL-Router fragt statt eine zweite Liste zu pflegen — **eine zweite Liste wäre eine zweite Wahrheit.**
- **Die Adresse ist das stärkere Signal als das Cookie.** `LocalePrefsMiddleware` sollte laut eigener Beschreibung nur die präfixlose **Startseite** anhand der gemerkten Sprache umleiten; die Bedingung traf bis zum 05.09.2026 **jede** präfixlose Adresse. Wer einmal auf `/en/` war, wurde beim Klick auf einen deutschen Link zurückgeworfen. Bots waren immer ausgenommen — deshalb stand es in keiner SEO-Messung und hat nur Besucher gestört.
- **Ein Honigtopf, der `hp` heißt, ist keiner.** Genau die Bots, gegen die er gebaut ist, überspringen ein Feld mit diesem Namen. Seit dem 05.09.2026 heißt er `website` — das, wonach ein Formularausfüller sucht. `views._honigtopf()` nimmt beide Namen an.
...
- **Kein Tracking, keine externen Requests vor Einwilligung.** Schriften selbst gehostet, Spline lädt erst nach `wvm_consent=all`, Cloudinary nur nutzerinitiiert. Notwendige Cookies: `wvm_lang`, `wvm_consent`, `csrftoken`. **Nur zwei davon kommen vom Server** — `csrftoken` und `wvm_lang`, und beide tragen seit dem 05.09.2026 `Secure`, `HttpOnly` und `SameSite=Lax`. `wvm_consent` setzt und liest der Cookie-Banner im Browser; es darf deshalb gerade **nicht** `HttpOnly` sein. Wer das verwechselt, schaltet den Banner ab. Ein Google-Tag bräuchte eine neue Einwilligungsstufe und einen Eintrag in der Datenschutzerklärung.
...
| **`../README.md`** | **Am 05.09.2026 neu geschrieben** und wieder brauchbar: Kurzfassung für den ersten Blick ins Repository, die Befehle und drei Regeln. Die frühere Fassung (09.07.2026) beschrieb die Dark-Landingpage einer „Digitalagentur" vor Umbau und Relaunch. Der gültige Einstieg zum Arbeiten bleibt `../CLAUDE.md` |
| **`../docs/` hat sechzehn Dateien** | Fünf davon sind **abgeschlossene Momentaufnahmen** und als Anleitung wertlos. Welche das sind und was daran noch gilt, steht in `../docs/00-INDEX.md`. Was zuletzt passiert ist: `../docs/LOGBUCH.md` |
| **`docs/UMBAU-START.md` und `docs/RELAUNCH-START.md`** | Beide sind abgeschlossene Momentaufnahmen (28.08.2026) und tragen veraltete URL-Zahlen (6, 57, 87). Aktuell sind 165. Das Design-System in `UMBAU-PLAN.md` §2 gilt dagegen weiter |
...
| **Der lokale Python ist neuer als die Produktion** | Auf dem Rechner liegt **Python 3.14** mit **Django 6.0.5**; deployt wird **Python 3.12** mit **Django 5.0.6** (`runtime.txt`, `requirements.lock`). `manage.py test` prüft also nicht das, was live läuft. Und **Django 5.0.6 lässt sich auf Python 3.14 gar nicht betreiben** — der Testlauf endet mit 428-mal `AttributeError: 'super' object has no attribute 'dicts'` aus `django/template/context.py`. Das ist kein Fehler im Projekt. Wer gegen die Produktionsfassung prüfen will, braucht eine Umgebung mit Python 3.12 oder 3.13: `py -3.13 -m venv …`, dann `pip install -r requirements.lock`. Am 05.09.2026 so geprüft: 122 Tests grün, alle vier Prüfbefehle grün |
| **Icons ändern** | Seit dem 05.09.2026 stehen die Formen **einmal** in `templates/icons_sprite.html` als `<symbol>`; `templates/icons.html` ist nur noch der `<use>`-Verweis. Wer ein Icon in `icons.html` sucht, findet dort nichts mehr. Der Aufruf `{% include 'icons.html' with name='web' %}` ist unverändert |
| **Änderungsdaten** | `landing/stand.py` wird **erzeugt** (`manage.py stand_schreiben`), nicht von Hand gepflegt. Wer Inhalte ändert und den Befehl vergisst, liefert ein `lastmod` aus, das nicht mehr stimmt — der CI-Lauf meldet es |
| **Inline-Skripte** | Jeder `<script>`-Block ohne `nonce="{{ request.csp_nonce }}"` wird vom Browser **nicht ausgeführt**. Die Content-Security-Policy wird durchgesetzt, nicht nur berichtet — seit dem 05.09.2026 meldet `landing/tests/test_csp.py` einen vergessenen Nonce auf jeder Adresse, statt dass man es im Browser bemerken muss. Datenblöcke (`application/json`, `application/ld+json`) brauchen keinen: Der Browser führt sie nicht aus |
...
| 3 | `G1` gilt seit 29.08.2026 als „Antwort-zuerst auf allen 87 Seiten durchgezogen" — Messung: **14 von 158** Seiten mit zitierfähiger Antwort (`GE23`) | Beide stimmen: Der Absatz ist überall vorhanden, aber er enthält oft **weder Zahl noch Definition**. Die Regel prüft die Zitierfähigkeit, nicht die Existenz. Echter Mangel, grösster GEO-Hebel. **Am 05.09.2026 für 14 Glossareinträge und 4 Fachbeiträge nachgezogen** — jede eingesetzte Zahl stammt aus dem Eintrag selbst oder aus `ANGEBOT_GROUPS`; zwei Beiträge bleiben bewusst ohne Zahl, weil sie dort erfunden werden müsste |
...
| 5 | Eigenmessung 27.08.2026: alle Kontraste ≥ 4,5:1, schwächster Wert 5,47:1 — Lighthouse: **Kontrastprüfung 0 %, 32 betroffene Elemente** (`BF18`) | **Aufgelöst, in zwei Schritten — und beide Male lag der Fehler dort, wo die Eigenmessung nicht hinsah.** Sie rechnete Token gegen Grund. **06.09.2026:** `--ink-dim` hielt **hell** nur 3,40:1 (dunkel 6,04:1) — die Eigenmessung hatte nur eine der beiden Fassungen erfasst; korrigiert auf `#746c64`, danach meldete die Messung 15 Elemente. **12.09.2026:** Vom Rest war **eines** übrig, `.err-code` auf der 404- und der 500-Seite — richtige Farbe, aber `opacity:.5` darüber. Eine Deckkraft unter 1 ist eine Farbänderung, und die kommt in keiner Token-Rechnung an: effektiv `#755a24` auf `#12100c`, 2,94:1 statt 8,41:1. Die Deckkraft ist entfernt, `ErrCodeKontrastTest` rechnet die Mischung seither nach. **Merksatz:** Eine Eigenmessung an Tokens beweist nichts über das, was auf dem Bildschirm ankommt — Deckkraft, Fassung und Grund gehören mitgerechnet |
...
| 15 | Messung: **„keine durchgesetzte CSP auf 158 von 158 Seiten"** (`SI08`) und **„keine einzige Testfunktion"** (`PJ02`) — im Code steht beides seit dem 05.09.2026 | Kein Widerspruch, sondern ein Zeitversatz: Die Messwerte stammen vom 02.09.2026, gebaut wurde am 05.09. Der richtige Umgang damit ist **nicht**, den Rat ein zweites Mal umzusetzen — ein zweiter CSP-Kopf wäre kein Fortschritt, und Tests zu schreiben, damit eine Messung sie zählt, wäre Beschäftigung ohne Wirkung. Stattdessen wurde der Zustand am 05.09. durch Prüfungen festgehalten (`test_csp.py`, `test_cookies.py`) und die Ausnahme in [80-AUFGABEN.md](80-AUFGABEN.md) eingetragen |
