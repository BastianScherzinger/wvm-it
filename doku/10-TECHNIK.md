...
status: teilweise
fortschritt: 85
zusammenfassung: 18.09.2026, auf dem Zweig sofort/2026-09-18-bt11 (5079058, nicht gepusht): BT11 -- die Gesundheitsadresse antwortet jetzt unter /health und /health/ mit ok, dieselbe View ohne Umleitung und bewusst ohne Datenbankabfrage, weil die Seite ohne Datenbank rendert. Beide Host-Weichen lassen /health/ durch, weil sie per startswith pruefen. railway.json setzt keinen healthcheckPath (Offen Nr. 9). Suite nachgezaehlt 328 Testfunktionen in 23 Dateien, laut Bausitzung gruen. Davor 17.09.2026, auf dem Zweig sofort/2026-09-17-fo08-und-1-weitere: FO08 -- jeder Anfrageweg (Kontakt, Konfigurator, Richtangebot, Kooperation, Newsletter, Website-Bogen) zaehlt seinen Abschluss serverseitig und cookielos ueber landing/messung.py, bewusst ohne gtag. FO10 anders als vorgeschlagen -- Kontaktformular, Konfigurator und Newsletter lehnen ohne angehaktes Pflichtkaestchen einwilligung ab (vorher pruefte nur der Browser, der Konfigurator traegt novalidate); die gemessene Stelle leistung_anfrage bleibt unveraendert, weil ihr Kaestchen die freiwillige Werbeeinwilligung ist und eine Ablehnung sie koppeln wuerde. Suite nachgezaehlt 326 Testfunktionen in 23 Dateien; die neuen sind laut Bausitzung nicht gelaufen (Offen Nr. 8). Davor am selben Tag: keine automatischen Mails mehr an eingetippte Adressen (`67f040e`). Seit 16.09.2026 laeuft das Projekt auf Django 5.2.17 (LTS) statt 5.0.6, dessen Reihe laut Testkopf seit dem 30.04.2025 keine Sicherheitskorrekturen mehr bekam -- gemeldet hatte das im Projekt nichts (SI40); liegt auf dem Zweig sofort/2026-09-16-si40-und-2-weitere. Die Lockfile-Zeile ist von Hand gesetzt, ob die Suite auf 5.2.17 gruen ist, zeigt erst der CI-Lauf (Offen Nr. 7). test_abhaengigkeiten.py haelt das Pflegeende der Reihe 5.2 als Datum fest und wird rot, sobald es erreicht ist (SI41). FO06 anders als vorgeschlagen, ohne Django-Form-Klasse: _ist_email prueft zusaetzlich mit validate_email und gegen die Laengengrenze, das Kontaktfeld der Kurzanfrage und die Felder des Detailbogens sind begrenzt, und anfrage_absenden prueft die Adresse aus dem signierten Token. Suite jetzt 314 Testfunktionen in 21 Testdateien (nachgezaehlt am 16.09.2026). Davor seit 12.09.2026 303 Testfunktionen in 20 Testdateien: dazugekommen sind acht Pruefungen fuer BF18 und eine neue Datei test_zahlen.py fuer GE25. Die BF18-Pruefungen pruefen etwas, das eine Token-Pruefung bauartbedingt nicht sieht -- zuletzt, dass die als Grafikfarbe deklarierte zweite Goldstufe --accent2 keinen Text mehr faerbt (sie tat es 28-mal) und dass .on-dark beide Goldstufen mit derselben Farbe belegt, weil nur deshalb die Umstellung im Dunkeln gefahrlos war. test_zahlen.py verlangt auf jeder Adresse mindestens eine Zahl mit Bezug im Fliesstext und haelt die Zahl im Antwortabsatz der beiden nur-deutschen Hubs gegen die Laenge ihrer Strukturliste: /aktuelles/ versprach 15 Fachbeitraege, waehrend das Modul 18 fuehrte -- zwei Stellen fuer dieselbe Aussage, und eine davon veraltet still. Am selben Tag VL02 als nicht anwendbar eingetragen, ohne eine Zeile Code: Es ist dieselbe blinde Stelle wie bei PJ11, das Pruefwerkzeug liest keine .lock-Dateien ein, und eine Umbenennung traefe die Namensliste ebenso wenig. Am selben Tag SI03 als beim Kunden liegend eingetragen und eine falsche Angabe dieser Datei berichtigt: includeSubDomains ist seit dem 05.09.2026 vorbelegt (settings.py:261), offen ist allein preload -- und das bleibt aus, solange die Apex-Domain wvm-it.tech beim Registrar parkt, weil ein Eintrag in die Preload-Liste sie dauerhaft unerreichbar machte statt nur tot (Begruendung im Code, settings.py:262-266; Offen Nr. 6, haengt an Beim Kunden Nr. 3). Zuletzt fielen im Kontrasttest die ausgeklammerten Zustaende weg -- drei Regeln setzten Gold als Text erst auf Eingabe, und :focus-visible ist der Zustand, in dem eine Tastaturbedienung dauerhaft steht. Die zwei juengsten davon sichern eine Regel, die seit dem Umbau 2026-08 im Kopf von style.css stand und nie geprueft wurde -- Gold als Text auf Hell nur ueber --accent-ink; zwei Regeln hielten sich nicht daran. GoldAlsTextTest liest dabei die Eigenschaft, nicht die Zeichenkette, weil accent-color und border-color auf dieselben fuenf Buchstaben enden. Im selben Durchgang wurde landing/stand.py ausnahmsweise von Hand nachgezogen, weil in der Sitzung kein Python lief -- nachgerechnet ueber die Quellenzuordnung des Befehls, nicht geschaetzt. Eine Deckkraft unter 1 ist eine Farbaenderung; .err-code stand auf --accent mit opacity:.5 und kam so auf 2,94:1 statt 8,41:1, durch jede bisherige Pruefung hindurch. ErrCodeKontrastTest rechnet jetzt die Mischung nach. Am selben Tag VL21 als Ausnahme eingetragen, aus demselben Grund wie VL01 am 11.09.: Wer einen Baustein verschiebt, den 13 Vorlagen einbinden, gibt ueber stand_schreiben (git log ohne --follow) 140 der 198 Adressen ein Aenderungsdatum ohne Inhaltsaenderung. Seit 11.09.2026 bindet requirements.txt das Lockfile per --constraint ein (PJ11, inzwischen auf main) -- vorher galt requirements.lock nur im CI-Lauf, weil Nixpacks ausschliesslich aus requirements.txt installiert, und jeder Deploy holte die mittelbaren Pakete in der Tagesfassung; ein echter Railway-Bau mit der Zeile ist gemergt, aber noch nicht im Build-Log nachgesehen. Am 11.09. ebenso VL01 als Ausnahme eingetragen: Ein Umzug nach apps/ setzte das Aenderungsdatum aller Seiten auf den Umzugstag. Bis 16.09.2026 Django 5.0.6 auf Railway; seit 10.09.2026 gibt es eine Testdatei fuer die Module, die bisher kein Test beruehrt hat (PJ03), und ihre Modulnamen stehen je in eigener Zeile, weil eine Quelltext-Analyse an einem gebuendelten Import nur das Paket sieht und die Pruefung deshalb nicht als Beleg zaehlte. Serverseitige Reichweitenmessung ohne Cookie und ohne IP; die Sicherung, die eine Anfrage vor dem Mailversand auf die Platte und ins Log schreibt, haengt seit dem 07.09. an allen Formularwegen statt nur an den Kurzanfragen (MW18). Dazu ConditionalGetMiddleware und Cache-Koepfe auf den maschinellen Endpunkten. HTML wird bewusst nicht zwischengespeichert: Das CSRF-Token wird je Anfrage neu maskiert. Lokale Pruefumgebung: py -3.13.
offen: 8
...
| Django | **5.2.17 (LTS)** — seit 16.09.2026 (`SI40`), vorher 5.0.6; die Reihe 5.0 bekommt laut Testkopf seit dem 30.04.2025 keine Korrekturen mehr. Pflegeende der Reihe 5.2 laut `landing/tests/test_abhaengigkeiten.py` der 30.04.2028 (`SI41`). ⚠ Liegt auf `sofort/2026-09-16-si40-und-2-weitere`, noch nicht auf `main` | `requirements.txt`, `requirements.lock` |
...
```bash
python -X utf8 manage.py test landing.tests   # 328 Tests (18.09.2026)
```
...
| ~~3~~ | ~~`apps/`-Struktur und reine Datenmodule (`data/`)~~ | `VL01` | **Am 11.09.2026 als Ausnahme eingetragen** ([80-AUFGABEN.md](80-AUFGABEN.md), „Bewertung der Messpunkte“). `manage.py` und `config/settings.py`, `urls.py`, `wsgi.py` liegen schon an der vereinbarten Stelle. Ein Umzug nach `apps/landing/` kostete die echten Änderungsdaten: `stand_schreiben` datiert jede Seite über `git log -1 -- <datei>` **ohne `--follow`** (`landing/management/commands/stand_schreiben.py:155–157`), danach trügen alle Adressen in Sitemap und Schema den Umzugstag. `INSTALLED_APPS` führt ausser `staticfiles` nur `landing`, ein `apps/`-Ordner hätte nichts zu ordnen. Die Inhaltskonstanten stehen schon heute in Modulen ganz ohne Import (`leistungen.py`, `branchen.py`, `glossar.py` u. a.); ein Ordner `data/` nur für den Zähler der Messung brächte nichts |
| 4 | Seitencache für die Ansichten ohne Formular | `PF10`, `BT04` | nicht begonnen; der größte verbliebene Hebel bei der Antwortzeit |
| 5 | Erster Railway-Bau mit `--constraint requirements.lock` | `PJ11` | Gebaut am 11.09.2026 (Commit `d87313f`), **am selben Tag auf `main` gemergt** (`0642f58`; `git branch --contains d87313f` nennt am 12.09.2026 `main`). Geprüft ist die Datei laut Commit mit pips eigenem Parser und mit der Suite, **nicht** mit einem Nixpacks-Bau; ein Push auf `main` deployt automatisch, ins Build-Log hat aber noch niemand gesehen. Also im Build-Log von Railway nachsehen, ob pip die Beschränkungen gelesen hat und der Dienst startet. Die Messregel wertet das Lockfile nicht, weil das Code-Audit keine `.lock`-Dateien einliest (Prüfwerkzeug `cockpit/pruefer/codeaudit.py:47–52`, `katalog/r_projekt.py:365–371`). Das liegt am Werkzeug, an der Seite ist dafür nichts zu tun. **Am 12.09.2026 gilt das auch für die Schwesterregel `VL02`** („Deploy- und Startdateien vollständig"): Sie zählt vier der fünf Dateien selbst als erfüllt und vermisst nur das Lockfile — dieselbe blinde Stelle, damit sind **alle vier** Namen, die sie als Lockfile gelten lässt, für sie unsichtbar. Als Ausnahme eingetragen, ohne eine Codezeile ([80-AUFGABEN.md](80-AUFGABEN.md), „Bewertung der Messpunkte"); eine Umbenennung in `requirements-lock.txt` träfe die Namensliste ebenso wenig und hinterliesse dieselbe Aussage an zwei Stellen — der Fehler, gegen den `start.sh` seit dem 05.09.2026 ausdrücklich gebaut ist |
| 6 | HSTS mit `preload` — **hängt am Kunden, nicht am Code** | `SI03` | **Am 12.09.2026 geprüft und als „beim Kunden" eingetragen** ([80-AUFGABEN.md](80-AUFGABEN.md), „Bewertung der Messpunkte"). Die eine Hälfte des Punktes steht seit dem 05.09.2026: `SECURE_HSTS_INCLUDE_SUBDOMAINS` ist auf `True` vorbelegt (`config/settings.py:261`). `SECURE_HSTS_PRELOAD` bleibt aus, solange die Apex-Domain `wvm-it.tech` beim Registrar parkt (A-Record `213.145.224.30`) — ein Eintrag in die Preload-Liste bindet Browser dauerhaft an HTTPS, für die Domain **und alle Subdomains**, und machte die kurze Adresse damit endgültig unerreichbar statt nur tot; die Begründung steht im Code (`:262–266`). Die Umgebungsvariable ist vorgesehen und muss nur auf `True` gezogen werden, sobald der CNAME beim Registrar steht ([80-AUFGABEN.md](80-AUFGABEN.md), „Beim Kunden" Nr. 3) |
| 7 | Django 5.2.17 im CI-Lauf und im Railway-Bau bestätigen | `SI40` | Gehoben am 16.09.2026 (Commit `8991195`), liegt auf `sofort/2026-09-16-si40-und-2-weitere`. Die Lockfile-Zeile ist von Hand gesetzt, der Auflösebefehl lief nicht; ob die Suite auf 5.2.17 grün ist, zeigt erst `.github/workflows/pruefen.yml`, das aus `requirements.lock` installiert. Nach dem Merge auf `main` zusätzlich im Railway-Build-Log nachsehen, ob der Dienst mit 5.2.17 startet (`start.sh` fährt `check --deploy --fail-level ERROR`) — zusammen mit Nr. 5 |
| 8 | Suite und `pruefe_sicherheit` für `FO08`/`FO10` laufen lassen | `FO08`, `FO10` | Gebaut am 17.09.2026 (Commits `f24bd1d`, `c33c7ee`), liegt auf `sofort/2026-09-17-fo08-und-1-weitere`. Laut Bausitzung lief kein Test, weil Python gesperrt war. Zu bestätigen: `python -X utf8 manage.py test landing.tests` und `python manage.py pruefe_sicherheit` — Letzterer sendet seit `c33c7ee` das Kästchen `einwilligung` mit, sonst würde der Server jede Probe ablehnen und die Mailzählung liefe ins Leere. Ebenso `stand_schreiben --pruefen`: Die Zeile für `/angebot/` in `landing/stand.py` ist in diesem Commit mitgezogen, ohne dass der Befehl lief |
| 9 | Railway-Healthcheck auf `/health` prüfen | `BT11` | Die Adresse steht seit 18.09.2026 unter `/health` und `/health/` (Commit `5079058`, Zweig `sofort/2026-09-18-bt11`, nicht gepusht). `railway.json` setzt aber keinen `healthcheckPath`, nur Startbefehl und Neustartregel. Ob der Dienst `wvm-it` im Projekt `webseiten` einen in den Einstellungen trägt, im Railway-Dashboard nachsehen. Ohne ihn geht ein Deploy live, sobald der Prozess läuft, nicht erst, wenn er antwortet ([80-AUFGABEN.md](80-AUFGABEN.md), „Offen" Nr. 20) |
