...
**295 Testfunktionen in 19 Testdateien** unter `landing/tests/` (nachgezählt am 12.09.2026 über `def test_`; dazu `__init__.py` und `_util.py`, die keine Prüfung tragen):
...
| `test_kontrast.py` (seit 06.09.2026, erweitert 12.09.2026 mit `BF18`) | jede Textfarbe hält 4,5:1 gegen jeden Grund, **in beiden Fassungen** — geprüft an den Tokens, nicht am Bildschirm. Dazu seit dem 12.09.2026 `ErrCodeKontrastTest` für den Fall, den eine Token-Prüfung bauartbedingt **nicht** findet: `.err-code` rechnet die Farbe **nach der Deckkraft** nach, hält den dunklen Kopf in beiden Fehlerseiten fest (die Rechnung gilt nur, solange er dunkel ist) und prüft die Mischfunktion gegen |
...
| **`opacity` auf Text** | Eine Deckkraft unter 1 ist eine **Farbänderung**, und `test_kontrast.py` sieht sie nicht: Es liest die Token, und die halten alle ihre 4,5:1. `.err-code` stand auf `--accent` mit `opacity:.5` und kam so auf 2,94:1 statt 8,41:1 — durch jede Prüfung gekommen und in der Messung vom 12.09.2026 das einzige beanstandete Element. Text dämpft man über ein Token (`--ink-soft`, `--ink-dim`), nicht über Deckkraft; neue Fälle gehören in `ErrCodeKontrastTest`, der die Mischung nachrechnet |
