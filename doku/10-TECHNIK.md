...
| 5 | Erster Railway-Bau mit `--constraint requirements.lock` | `PJ11` | Gebaut am 11.09.2026 (Commit `d87313f`), liegt auf `sofort/2026-09-11-vl01-und-2-weitere`. Geprüft ist die Datei laut Commit mit pips eigenem Parser und mit der Suite, **nicht** mit einem Nixpacks-Bau. Nach dem Merge auf `main` im Build-Log von Railway nachsehen, ob pip die Beschränkungen gelesen hat und der Dienst startet. Die Messregel wertet das Lockfile nicht, weil das Code-Audit keine `.lock`-Dateien einliest (Prüfwerkzeug `cockpit/pruefer/codeaudit.py:47–52`, `katalog/r_projekt.py:365–371`). Das liegt am Werkzeug, an der Seite ist dafür nichts zu tun |
