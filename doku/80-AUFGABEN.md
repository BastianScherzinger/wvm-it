...
status: teilweise
fortschritt: 82
zusammenfassung: Loop B hat am 07.09.2026 geprueft, was Loop A gebaut hat (haelt stand, live in allen drei Sprachen), die vier vom Werkzeug uebertragenen Punkte EIG01-EIG04 an dieser Seite nachgeprueft (EIG01 und EIG04 treffen hier NICHT zu, EIG02 und EIG03 schon) und die Seite auf die Hardware-Suchabsicht hin untersucht. Der groesste Fund ist struktureller Art: Von zwoelf echten Suchanfragen zum Geraetelebenszyklus haben sechs KEINE Seite -- alle 13 Leistungsseiten sind um laufende Betreuung gebaut, keine um ein einzelnes Geraet ohne Vertrag. Dazu zwei Rechtsfunde (Barrierefreiheitserklaerung widerspricht sich, Impressum verweist auf die abgeschaltete EU-Streitbeilegungsplattform) und ein Conversion-Fehler: Der Einstieg auf der EDV-Hauptseite kostet 490 Euro, obwohl die passende 190-Euro-Position im Katalog steht. Das Urteil zu BF18 wurde von 'bestanden' auf 'mit Auflagen' korrigiert.
offen: 12
...
| 4 | **Barrierefreiheitserklärung widerspricht sich selbst** — dringend, weil sie eine verbindliche Erklärung ist. Abschnitt 2 sagt „Die Textfarben erreichen ein Kontrastverhältnis von mindestens 4,5 zu 1"; Abschnitt 3 räumt im nächsten Absatz Werte „unter der Zielmarke" ein. **Beides steht direkt untereinander.** Seit dem 06.09. stimmt die Aussage für die Farb-Tokens (nachgewiesen von `test_kontrast.py`), aber Lauf #309 meldet weiterhin **15 Elemente**. Abschnitt 3 beschreibt zudem einen überholten Zustand („wird derzeit gegen eine eigene Messung abgeglichen") — der Abgleich ist erfolgt, die Abweichung hat sich bestätigt | Loop B 07.09., `content.json` → `barrierefreiheit` |
...
| 7 | **Impressum verweist auf die abgeschaltete OS-Plattform.** Der Text nennt `https://ec.europa.eu/consumers/odr`; die EU hat die Plattform zum **20.07.2025** eingestellt. Der Hinweis ist damit nicht nur überflüssig, sondern irreführend — er schickt Verbraucher an eine Stelle, die es nicht mehr gibt. Der zweite Halbsatz („nicht verpflichtet und nicht bereit") kann bleiben | Loop B 07.09., abgeleitet aus `EIG03` |
| 8 | **Zwei Video-Poster im alten Format**: `pc_poster.jpg` (47 KB) und `server_poster.jpg` (52 KB). Sie liegen ausserhalb der `<img>`-Prüfung von `test_bilder.py` und sind deshalb beim WebP-Durchgang am 07.09. durchgerutscht. `wvm_mark_128.png` bleibt zu Recht PNG — Apple-Touch-Icons können kein WebP | `PF15` (Rest) |
| 9 | **Der Einstieg auf `/leistungen/edv-it-betreuung/` ist der teuerste statt der naheliegendste.** `landing/leistungen.py:38` setzt `"einstieg": "sicherheitscheck"` — **490 €**. Im Katalog steht seit jeher die Position `arbeitsplatz` („Neuen Arbeitsplatz einrichten") für **190 €**. Wer auf der Hauptseite des EDV-Felds landet, weil er konkret einen Rechner eingerichtet haben will, bekommt als ersten Schritt eine Prüfung angeboten, die zweieinhalbmal so teuer ist wie das, was er sucht. Ein Zeilenwechsel, gemessene Wirkung | Loop B 07.09., `KV05` |
