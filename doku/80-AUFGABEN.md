...
| EIG04 | PageVisit speichert die Proxy-Adresse statt der Besucher-IP | technik | offen | an RTC-Service gefunden am 07.09.2026: apps/core/middleware.py:189` `_anonymize_ip(request.META.get('REMOTE_ADDR'))` — dafür gibt es `spam.get_client_ip()` (`apps/core/spam.py:69`), das genau diesen Fall behandelt und in beiden Formularpfaden benutzt wird. Hinter dem Railway-Proxy steht in jeder Zeile derselbe Wert. **An dieser Seite noch nicht geprüft.** | 2026-09-07 |
<!-- eigenepunkte:ende -->
