...
status: teilweise
fortschritt: 82
zusammenfassung: Loop A hat am 07.09.2026 vier der zwoelf von Loop B dokumentierten Punkte abgearbeitet (Commit 7b7207a, live geprueft): Der Einstieg auf der EDV-Hauptseite kostete 490 statt 190 Euro, obwohl der Kopf der Datei das Feld als 'kleiner erster Schritt' definiert. Zwei Rechtstexte behaupteten Ueberholtes -- das Impressum verwies auf die zum 20.07.2025 eingestellte EU-Streitbeilegungsplattform, und die Barrierefreiheitserklaerung widersprach sich in zwei aufeinanderfolgenden Abschnitten. Dazu zwei Video-Poster, die ausserhalb jeder Bildpruefung lagen, weil sie in content.json stehen. Offen bleibt der groesste Punkt: Sechs Suchanfragen zum Geraetelebenszyklus haben keine Seite -- der braucht einen eigenen Durchgang, weil er dreisprachig ist. 207 Tests.
offen: 8
...
| 10 | **Der Gerätelebenszyklus fehlt als Thema — sechs Suchanfragen ohne jede Seite.** Geprüft wurden zwölf echte Formulierungen; ohne Treffer sind: „Computer neu aufsetzen Firma", „alter PC langsam", „Datenübernahme alter Rechner", „PC Aufrüstung Arbeitsplatz", „Hardware Reparatur", „Windows 11 Umstellung Betrieb". „aufrüst" und „Datenübernahme" kommen im **ganzen Projekt null Mal** vor. Das ist kein Zufall, sondern Struktur: Alle 13 Leistungsseiten sind um laufende Betreuung, Sicherheit, Netzwerk oder Vor-Ort-Technik gebaut — **keine einzige um ein einzelnes Gerät, jetzt, ohne Vertrag.** Genau diese Absicht fällt bei einer Seite, die sich als externe IT-Abteilung positioniert, systematisch durch. Der Preis dafür existiert (190 €), er hat nur keine Seite | Loop B 07.09., `SU08`/`IS18` |
| 11 | **Windows-10-Ende ist der dringlichste Einzelfall daraus.** Windows 10 bekommt seit Oktober 2025 keine Sicherheitsupdates mehr; manche Betriebe laufen seit fast einem Jahr ohne. Der einzige verwandte Beitrag (`/aktuelles/alte-windows-version-im-betrieb/`) behandelt bewusst den Sonderfall isolierter Maschinensteuerungen und nennt **weder „Windows 10" noch „Windows 11" noch ein Enddatum**. Vorschlag: eigener Beitrag zur Standardfrage eines Büros — umstellen oder Gerät tauschen, TPM 2.0, Kosten aus `ANGEBOT_GROUPS` | Loop B 07.09. |
...
| EIG05 | „10+ Jahre Branchenerfahrung" gegen `foundingDate` 2024 im Schema derselben Seite | inhalte | offen | an Rümpelwerk Mitteldeutschland gefunden am 07.09.2026: templates/ueber-uns.html:229` („10+" / „Jahre Branchenerfahrung") und `:261` („10+ Jahre Branchenerfahrung"), live bestätigt; `apps/core/schema.py:251` `'foundingDate': '2024'`. Die Zahl ist zweimal getippt, hat keine Quelle in `apps/core/data/` und widerspricht dem Gründungsjahr, das jede Seite im JSON-LD mitliefert. **An dieser Seite noch nicht geprüft.** | 2026-09-07 |
| EIG06 | Die Löschung der Sitzungsdaten hängt hinter dem Mailversand — dieselbe Kopplung, die bei `PageVisit` schon einmal die Frist gerissen hat | technik | offen | an Rümpelwerk Mitteldeutschland gefunden am 07.09.2026: apps/core/views.py:1737–1754`: `send_daily_report()` bricht bei `email_sent=True` ab, ruft dann `_send_daily_report_email(...)` und **erst danach** `VisitorSession.objects.filter(date__lt=cutoff).delete()` (90 Tage). Wirft der Versand (Resend), wird nie gelöscht — genau die Ursache, die `apps/core/apps.py:70–77` für `PageVisit` beschreibt und dort behoben hat. Die 90 Tage stehen in keiner Zeile der Datenschutzerklärung; `templates/datenschutz.html:281` nennt nur das Cookie „rwsid" mit 30 Minuten Laufzeit. **An dieser Seite noch nicht geprüft.** | 2026-09-07 |
<!-- eigenepunkte:ende -->
