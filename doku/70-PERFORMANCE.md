...
<!-- tempo:anfang -->
**Messung vom 26.09.2026** (Webagentur Scherzinger Overview, Regelstand 2026-09-26a). Bereich „Performance & Core Web Vitals“: **98,0 von 100**, Reifegrad „Referenz“.
...
| `/` | desktop | **100** | 0,54 s | 0,000 | 6 ms | 5 ms |
| `/datenschutz/` | mobile | **100** | 1,66 s | 0,000 | 0 ms | 2 ms |
| `/datenschutz/` | desktop | **100** | 0,38 s | 0,000 | 0 ms | 2 ms |
| `/impressum/` | mobile | **100** | 1,67 s | 0,000 | 0 ms | 2 ms |
| `/impressum/` | desktop | **100** | 0,41 s | 0,000 | 0 ms | 9 ms |
| `/kontakt/` | mobile | **100** | 1,66 s | 0,000 | 0 ms | 2 ms |
| `/kontakt/` | desktop | **100** | 0,39 s | 0,000 | 0 ms | 3 ms |
| `/kosten/rechner/` | mobile | **99** | 2,01 s | 0,000 | 0 ms | 1 ms |
| `/kosten/rechner/` | desktop | **100** | 0,49 s | 0,000 | 0 ms | 3 ms |
| `/leistungen/` | mobile | **99** | 1,99 s | 0,000 | 0 ms | 3 ms |
| `/leistungen/` | desktop | **100** | 0,50 s | 0,000 | 3 ms | 3 ms |
...
| 5 | **Critical CSS** je Seitentyp inline, Hauptdatei asynchron. ~~HTML unter 120 KiB (`/`, `/en/`, `/ro/` liegen darüber)~~ — **am 24.09.2026 als begründete Ausnahme eingetragen** (Paket 343, `315811f`, [80-AUFGABEN.md](80-AUFGABEN.md), „Bewertung der Messpunkte"), ohne eine Zeile Code: Unter die Grenze käme die Startseite nur ohne den Konfigurator im ausgelieferten HTML — dieselbe Abwägung wie „T2 Startseite bewusst nicht verschlankt" oben. Offen bleibt der Critical-CSS-Teil | `PF14` (`VL16` Ausnahme) |
| ~~6~~ | ~~Statische Dateien mit `immutable` ausliefern~~ — **am 12.09.2026 als begründete Ausnahme eingetragen** ([80-AUFGABEN.md](80-AUFGABEN.md), „Bewertung der Messpunkte"), ohne eine Zeile Code. Das weit gesetzte Ablaufdatum steht längst; der Beleg der Messung nennt es selbst (`max-age=31536000, public` an beiden geprüften Dateien). Gemeldet wird allein das fehlende `immutable` — und das schliesst derselbe Rat in seinem zweiten Halbsatz aus: „ohne Hash im Namen darf das nicht sein." Die Namen tragen hier keinen Hash (siehe „Weiter umgesetzt", Zeile „Statische Dateien"). `immutable` heisst: frag nie wieder nach, auch nicht bedingt — unter der Adresse **ohne** `?v=`, die WhiteNoise ebenso ausliefert, sässe damit jeder Zwischenspeicher ein Jahr auf einem überholten CSS ohne Weg zurück. Wo es gefahrlos geht, ist es gebaut. `VL14` bleibt davon unberührt | `PF13`, `VL14` |
| 7 | **`PF28` bestätigen:** Suite (`test_verkleinern.py`) und ein `collectstatic` fahren, das verkleinerte `main.js` im Browser ohne Konsolenfehler laden — laut Bausitzung lief beides nicht. Nach dem Merge zeigt der nächste Messlauf, ob `main.js` als verkleinert gilt; die Regel könnte auch weiter anschlagen, weil Kommentare hinter Code und alle Zeilenumbrüche bewusst bleiben ([10-TECHNIK.md](10-TECHNIK.md), „Offen" Nr. 11) | `PF28` |
